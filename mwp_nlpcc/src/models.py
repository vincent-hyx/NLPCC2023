...
from transformers import AutoModel, BertModel, BertLMHeadModel, BertForMaskedLM

class AdapterWeight(nn.Module):
    def __init__(self):
        super(AdapterWeight, self).__init__()

        self.weight_gen_loss = nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.weight_mlm_loss = nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        self.weight_gen_loss.data.fill_(0.0)
        self.weight_mlm_loss.data.fill_(0.0)

    def forward(self, gen_loss, mlm_loss):
        # s_gen_loss = torch.log(self.weight_gen_loss.pow(2))
        # s_mlm_loss = torch.log(self.weight_mlm_loss.pow(2))
        loss = torch.exp(-self.weight_gen_loss) * gen_loss + torch.exp(-self.weight_mlm_loss) * mlm_loss + self.weight_gen_loss + self.weight_mlm_loss
        return loss
...
"""
edited by xu: use PLM to represent or encode linguistic description of the MWP
"""
class EncoderPLM(nn.Module):
    def __init__(self, hidden_size, auto_transformer=False, pretrain_path='', dropout=0.5):
        super(EncoderPLM, self).__init__()
        self.bert_model = None
        self.embedding_size = 768
        if auto_transformer:
            print("auto_model: ", pretrain_path)
            self.bert_model = AutoModel.from_pretrained(pretrain_path)
            self.embedding_size = hidden_size
        else:
            print("bert_model: ", pretrain_path)
            self.bert_model = BertModel.from_pretrained(pretrain_path, cache_dir="./PLMs") # Erniemodel or Bertmodel
        self.hidden_size = hidden_size
        self.dropout = dropout
...
class EncoderPLM_MLM(nn.Module):
    def __init__(self, hidden_size, auto_transformer=False, pretrain_path='', dropout=0.5):
        super(EncoderPLM_MLM, self).__init__()
        self.bert_model = None
        self.embedding_size = 768
        if auto_transformer:
            print("auto_model: ", pretrain_path)
            ...
            print("bert_model: ", pretrain_path)
            self.bert_model = BertForMaskedLM.from_pretrained(pretrain_path)
        self.hidden_size = hidden_size
        self.dropout = dropout
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        if labels is None:
            output = self.bert_model(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=True)
            last_hidden_state = output[1][-1]
            # print(last_hidden_state.shape)
            # hidden_state = output[0]
            embedded = self.em_dropout(last_hidden_state)  # B x S x Bert_emb(768)
            ...
            output = self.bert_model(input_ids=input_ids.contiguous(), attention_mask=attention_mask,
                                     output_hidden_states=True,
                                     labels=labels.contiguous())
            last_hidden_state = output[2][-1]
            # print(last_hidden_state.shape)
            embedded = self.em_dropout(last_hidden_state)  # B x S x Bert_emb(768)
            ...
            problem_output = pade_outputs[0]
            loss = output[0]
            return pade_outputs, problem_output, loss
