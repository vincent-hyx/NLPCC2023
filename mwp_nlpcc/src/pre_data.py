...
"""
edited by xu: read json format like this: [{},{},{}]
"""
...
"""
edit by xu: in the "transfer_num" function, I replace NUM with Ni to enable PLM add new token([Ni])
2023.4.14  
"""
def transfer_num(data):  # transfer num into "NUM"
    ...
    for d in data:
        ...
        num_pos = []
        token_list = ["N0", "N1", "N2", "N3", "N4", "N5", "N6", "N7", "N8", "N9", "N10", "N11", "N12", "N13", "N14"]
        for i, j in enumerate(input_seq):
            if j in token_list:
                num_pos.append(i+1) # 要考虑预训练的[cls]
        assert len(nums) == len(num_pos)
        # pairs.append((input_seq, out_seq, nums, num_pos, d["ans"]))
        if 'k' in d:
            pairs.append((input_seq, out_seq, nums, num_pos, d["k"], d["k_mask"]))
        else:
            pairs.append((input_seq, out_seq, nums, num_pos))

    temp_g = []
    for g in generate_nums:
        if generate_nums_dict[g] >= 3:
            temp_g.append(g)
    return pairs, temp_g, copy_nums


def transfer_num_gts(data):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
...
"""
edited by xu: in the "transfer_num_for_val_data" function, I replaces Ni with [Ni] to enable PLM add new token([Ni])
there is no equation in the validation data, so I replace it's equation with '+++' that done't mean anything to just
unified the input data format. Meanwhile, I must evaluate answer is right or not, so the function also return ground 
truth answer.  
"""

def transfer_num_for_val_data(data, test_flag=False):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
    ...
    for d in data:
        ...
        if test_flag is False:
            answer_list.append(eval(d["answer"])) # 用于NLPCC
            # answer_list.append(d["answer"]) # 用于测试论文中的数据
        else:
    ...
    return pairs, temp_g, copy_nums, answer_list


def transfer_num_for_val_data_gts(data):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
    ...
    for d in data:
        ...
        # a = re.sub(r"%", '/100', temp1)
        # b = re.sub("[0-9]*\(\([0-9]*\)/\([0-9]*\)\)", '0', a)
        # print(b, type(b))
        # temp2 = eval(b)
        # print(temp1)
        answer_list.append(eval(temp1))
        # print(type(float(d["ans"])), float(d["ans"]), temp2)
...
"""
edited by xu: add a new parameter called PLM_flag in original function prepare_data, and tag [NUM] token position in the 
sequence processed by PLM tokenizer
"""
def prepare_data_with_PLM_and_MLM(pairs_trained, pairs_tested, generate_nums, copy_nums, tokenizer, tree=False, PLM_flag=True):
    # input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_trained:
        ...
        # edited by xu: use PLM tokenizer
        if PLM_flag:
            s = ''
            for token in pair[0]:
                s = s + token
            ...
            # print(s, len(s))
            if pair[4] == "None":
                input_cell = tokenizer(s, padding='max_length', max_length=200, truncation=True)
                input_cell = [input_cell["input_ids"], input_cell['attention_mask']]
            else:
                input_cell = tokenizer(s, padding='max_length', max_length=200, truncation=True)
                input_cell = [input_cell["input_ids"], input_cell['attention_mask']]

                input_s = s + pair[5]
                mlm_input = tokenizer(input_s, padding='max_length', max_length=200, truncation=True)
                label_s = s + pair[4]
                label = tokenizer(label_s, padding='max_length', max_length=200, truncation=True)
                mlm_input_cell = [mlm_input["input_ids"], mlm_input['attention_mask'], label["input_ids"]]
                mlm_train_pairs.append(mlm_input_cell)
        else:
            input_cell = []
            print("please check two parameters:PLM_flag and tokenizer")
        ...
        num_pos = []
        token_list = ["N0", "N1", "N2", "N3", "N4", "N5", "N6", "N7", "N8", "N9", "N10", "N11", "N12", "N13", "N14"]
        num_token_id = tokenizer.convert_tokens_to_ids(token_list)
        sep_id = tokenizer.convert_tokens_to_ids("[SEP]")
        sep_id_index = -1
        for i, j in enumerate(input_cell[0]):
            if j == sep_id:
                sep_id_index = i
                break
            if j in num_token_id:
                num_pos.append(i)  #
        # print(len(pair[3]), len(num_pos))
        assert len(pair[3]) == len(num_pos)
        # edited by xu : we mask some token in second sentence(prefix equation)

        train_pairs.append((input_cell, len(input_cell[0]), output_cell, len(output_cell),
                            pair[2], num_pos, num_stack))
    ...
    for pair in pairs_tested:
        ...
        if PLM_flag:
            ...
            for token in pair[0]:
                s = s + token
            input_cell = tokenizer(s, padding='max_length', max_length=160, truncation=True)
            input_cell = [input_cell["input_ids"], input_cell['attention_mask']]
        else:
            input_cell = []
            print("please check two parameters:PLM_flag and tokenizer")
        ...
        for i, j in enumerate(input_cell[0]):
            if j in num_token_id:
                num_pos.append(i)
        assert len(pair[3]) == len(num_pos)
        test_pairs.append((input_cell, len(input_cell[0]), output_cell, len(output_cell),
                           pair[2], num_pos, num_stack))
    print('Number of testing data %d' % (len(test_pairs)))
    return output_lang, train_pairs, test_pairs, mlm_train_pairs


def prepare_test_data(pairs_tested, output_lang, tokenizer, PLM_flag=True, tree=True):
    test_pairs = []
    for pair in pairs_tested:
        num_stack = []
        for word in pair[1]:
...
def prepare_data_with_PLM(pairs_trained, pairs_tested, generate_nums, copy_nums, tokenizer, tree=False, PLM_flag=True):
    # input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_trained:
        ...
        # edited by xu : we mask some token in second sentence(prefix equation)
        # mask_pos = []
        # for i in range(sep_id_index + 1, len(input_cell[1])):
        #     if input_cell[0][i] == sep_id:
        #         break
        #     else:
        #         np.random.seed(i)
        ...
        train_pairs.append((input_cell, len(input_cell[0]), output_cell, len(output_cell),
                            pair[2], num_pos, num_stack))
        # # edited by xu: we re-generate other data with all token in prefix equation masked
        # new_input_cell = copy.deepcopy(input_cell)
        # for t in range(sep_id_index+1, len(new_input_cell[1])):
        #     if new_input_cell[0][t] == sep_id:
        #         break
        #     else:
        #         new_input_cell[1][t] = 0
        # train_pairs.append((new_input_cell, len(new_input_cell[0]), copy.deepcopy(output_cell), len(output_cell),
        #                     copy.deepcopy(pair[2]), copy.deepcopy(num_pos), copy.deepcopy(num_stack)))
    # print('Indexed %d words in input language, %d words in output' % (input_lang.n_words, output_lang.n_words))
    print('Number of training data %d' % (len(train_pairs)))
    for pair in pairs_tested:
        num_stack = []
...
def prepare_data_with_PLM_MASK(pairs_trained, pairs_tested, generate_nums, copy_nums, tokenizer, tree=False, PLM_flag=True):
    # input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_trained:
        ...
        if PLM_flag:
            ...
            for token in pair[1]:
                eq = eq + token
            # print(s, len(s))
            input_cell = tokenizer(s, eq, padding='max_length', max_length=200, truncation=True)
            ori_input = input_cell["input_ids"]
            ori_input_att = input_cell['attention_mask']
        else:
            input_cell = []
            print("please check two parameters:PLM_flag and tokenizer")
...
def prepare_train_batch(pairs_to_batch, batch_size):
    ...
    for batch in batches:
        ...
        for i, li, j, lj, num, num_pos, num_stack in batch:
            ...
            seq = pad_seq(j, lj, output_len_max)
            # print(len(seq))
            output_batch.append(seq)
