...
def compute_val_result(test_res, test_tar_value, output_lang, num_list, num_stack):
    if len(num_stack) == 0 and test_res == test_tar_value:
        return True
    test = out_expression_list(test_res, output_lang, num_list)
    # print(test)
    # print(test_tar_value)
    # tar = out_equation(test_tar, output_lang, num_list, copy.deepcopy(num_stack))
    tar = test_tar_value
    test_val = compute_prefix_expression(test)
    if test is None:
        return False, test, test_val
    try:
        if abs(test_val - tar) < 1e-4:
            return True, test, test_val
        else:
            return False, test, test_val
    except:
        return False, test, test_val
...
def get_all_number_encoder_outputs(encoder_outputs, num_pos, batch_size, num_size, hidden_size):
    ...
    # print(f"{indices}, {all_embedding.shape}")
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    # Turn padded arrays into (batch_size x max_len) tensors, transpose into (max_len x batch_size)
    # input_var = torch.LongTensor(input_batch).transpose(0, 1)
    input_var = torch.LongTensor(input_batch)# [B x 2 X max_len]
    ...
    encoder_outputs, problem_output = encoder(input_var[:, 0, :], input_var[:, 1, :])
    # Prepare input and output variables
...
def train_tree_for_PLM(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    encoder_outputs, problem_output = encoder(input_var[:, 0, :], input_var[:, 1, :])
    # print(encoder_outputs.shape, problem_output.shape)
    # Prepare input and output variables
    node_stacks = [[TreeNode(_)] for _ in problem_output.split(1, dim=0)]
    ...
    for t in range(max_target_length):
        ...
        # all_leafs.append(p_leaf)
        outputs = torch.cat((op, num_score), 1)
        # _, j = torch.softmax(outputs, 1).topk(1)
        # print(op.shape)
        # print(num_score.shape)
        # print(num_score)
        # print(outputs.shape)
        all_node_outputs.append(outputs)
        ...
        for idx, l, r, node_stack, i, o in zip(range(batch_size), left_child.split(1), right_child.split(1),
            ...
            if i < num_start:
                ...
                # print(current_nums_embeddings.shape)
                # print(all_nums_encoder_outputs.shape)
                current_num = current_nums_embeddings[idx, i - num_start].unsqueeze(0)
                while len(o) > 0 and o[-1].terminal:
                    sub_stree = o.pop()
                    op = o.pop()
    ...
    # Update parameters with optimizers
    # final_loss = loss_CLM + loss.item()

    return loss.item()  # , loss_0.item(), loss_1.item()


def train_tree_for_PLMandMLM(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, output_lang, num_pos, mlm_data, english=False):
    # sequence mask for attention
    seq_mask = []
    max_len = max(input_length)
    for i in input_length:
    ...
    # Run words through encoder
    mlm_input = torch.LongTensor(mlm_data).cuda()
    _, _, mlm_loss = encoder(mlm_input[:, 0, :], mlm_input[:, 1, :], mlm_input[:, 2, :])
    ...
    # op_target = target < num_start
    # loss_0 = masked_cross_entropy_without_logit(all_leafs, op_target.long(), target_length)
    generate_loss = masked_cross_entropy(all_node_outputs, target, target_length)
    # loss = loss_0 + loss_1
    # # adapter weight
    # s_gen_loss = torch.log(weight_gen_loss.pow(2))
    # s_mlm_loss = torch.log(weight_mlm_loss.pow(2))
    # loss = 1/2*torch.exp(-s_gen_loss)*generate_loss + 1/2*torch.exp(-s_mlm_loss)*mlm_loss + s_mlm_loss + s_gen_loss
    ...
    return generate_loss, mlm_loss  # , loss_0.item(), loss_1.item()
...
def evaluate_tree(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
    ...
    for t in range(max_length):
        ...
        while len(beams) > 0:
            ...
            # op_score = nn.functional.softmax(op, dim=1)
            # num_score = nn.functional.softmax(num_score, dim=1)
            # max_pro_op = op_score.topk(1)[0]
            # max_pro_num = num_score.topk(1)[0]
            # if max_pro_op > max_pro_num:
            #     out_score = torch.cat((op_score, torch.zeros_like(num_score)), dim=1)
            # else:
            #     out_score = torch.cat((torch.zeros_like(op_score), num_score), dim=1)
            # print(f"op_score:{op_score}")
            # print(f"num_score:{num_score}")
            # print(f"out_score:{out_score}")
...
def train_tree_gts(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, output_lang, num_pos, english=False):
    # sequence mask for attention
    seq_mask = []
...
def evaluate_tree_gts(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
                  beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
